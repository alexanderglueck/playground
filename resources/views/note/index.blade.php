                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <ul>
                        @foreach($notebooks as $notebook)
                            <li>
                                <a href="{{ route('notebooks.show', [$notebook->id]) }}" title="{{ __('View notebook') }}">{{ $notebook->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                    ...
                    <ul class="list-inline">
                        @foreach($tags as $tag)
                            <li class="list-inline-item">
                                <a class="badge badge-pill badge-{{ $tag->pill() }}" href="{{ route('tags.show', [$tag->id]) }}" title="{{ __('View tag') }}">{{ $tag->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
